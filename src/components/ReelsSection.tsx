  ...
  return (
    <div className="relative h-[420px] w-[234px] shrink-0 snap-center overflow-hidden rounded-2xl bg-black">
      ...
      <div className="absolute bottom-3 left-3 right-3 text-white">
        <p className="text-sm font-semibold leading-tight">{reel.creator}</p>
        {reel.handle && (
          <p className="text-xs text-white/80">{reel.handle}</p>
        )}
      </div>
      {/* Mute toggle */}
      <button
...
export default function ReelsSection({ reels = DEFAULT_REELS }: { reels?: Reel[] }) {
  ...
  return (
    <section className="py-6">
      <h2 className="mb-3 px-4 text-lg font-bold text-gray-800">
        Creator Reels
      </h2>
      <div
        ref={scrollRef}
